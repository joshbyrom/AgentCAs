...
var View = function(cellspace, canvas_id) {
	...
	this.set_width_height = function() {
		this.cell_width = this.canvas.width / this.canvas.number_of_columns;
		this.cell_height = this.canvas.height / this.canvas.number_of_rows;
	...
	this.draw_cell = function(cell) {
		...
		var color = cell.get_state() == 'empty' ? 'white' : cell.agent.get_color();
...
var GroupLogic = function(cellspace, groups) {
	...
	this.handle_cell = function(cell) {
		...
		if(state === 'unknown') {
			if(Math.random() < this.birth_change) {
				cell.agent = this.create_agent();
				cell.next_state = 'occupied';
			} else cell.next_state = 'empty';
	    }
	};
...
var main = function(canvas_id) {
	...
	var cellspace = new Cellspace(24, 24);
